...
/// [NFPopupMenuItem]), or no value at all (for example, [NFPopupMenuDivider]).
///
/// See also:
///
///  * [NFPopupMenuItem], a popup menu entry for a single value.
///  * [NFPopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [NFCheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [PopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
abstract class NFPopupMenuEntry<T> extends StatefulWidget {
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const NFPopupMenuEntry({Key key}) : super(key: key);
...
/// See also:
///
///  * [NFPopupMenuItem], for the kinds of items that this widget divides.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [PopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
// ignore: prefer_void_to_null, https://github.com/dart-lang/sdk/issues/34416
class NFPopupMenuDivider extends NFPopupMenuEntry<Null> {
  /// Creates a horizontal divider for a popup menu.
  ///
  /// By default, the divider has a height of 16 logical pixels.
  const NFPopupMenuDivider({Key key, this.height = _kMenuDividerHeight})
      : super(key: key);
...
class _PopupMenuDividerState extends State<NFPopupMenuDivider> {
  @override
...
/// [NFCheckedPopupMenuItem].
///
/// Typically the [child] of a [NFPopupMenuItem] is a [Text] widget. More
/// elaborate menus with icons can use a [ListTile]. By default, a
/// [NFPopupMenuItem] is 48 pixels high. If you use a widget with a different
...
/// complete menu, and see the example at [NFCheckedPopupMenuItem] for one way to
/// keep the text of [NFPopupMenuItem]s that use [Text] widgets in their [child]
/// slot aligned with the text of [NFCheckedPopupMenuItem]s or of [NFPopupMenuItem]
/// that use a [ListTile] in their [child] slot.
///
/// See also:
///
///  * [NFPopupMenuDivider], which can be used to divide items from each other.
///  * [NFCheckedPopupMenuItem], a variant of [NFPopupMenuItem] with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [PopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class NFPopupMenuItem<T> extends NFPopupMenuEntry<T> {
  /// Creates an item for a popup menu.
  ///
  /// By default, the item is [enabled].
  ///
  /// The [height] and [enabled] arguments must not be null.
  const NFPopupMenuItem({
    Key key,
    this.value,
    this.enabled = true,
  ...
  @override
  NFPopupMenuItemState<T, NFPopupMenuItem<T>> createState() =>
      NFPopupMenuItemState<T, NFPopupMenuItem<T>>();
}

/// The [State] for [NFPopupMenuItem] subclasses.
///
/// By default this implements the basic styling and layout of Material Design
/// popup menu items.
///
/// The [buildChild] method can be overridden to adjust exactly what gets placed
/// in the menu. By default it returns [NFPopupMenuItem.child].
///
/// The [handleTap] method can be overridden to adjust exactly what happens when
/// the item is tapped. By default, it uses [Navigator.pop] to return the
/// [NFPopupMenuItem.value] from the menu route.
///
/// This class takes two type arguments. The second, `W`, is the exact type of
/// the [Widget] that is using this [State]. It must be a subclass of
/// [NFPopupMenuItem]. The first, `T`, must match the type argument of that widget
/// class, and is the type of values returned from this menu.
class NFPopupMenuItemState<T, W extends NFPopupMenuItem<T>> extends State<W> {
  /// The menu item contents.
  ///
  /// Used by the [build] method.
  ///
  /// By default, this returns [NFPopupMenuItem.child]. Override this to put
  ...
  ///
  /// By default, uses [Navigator.pop] to return the [NFPopupMenuItem.value] from
  /// the menu route.
  @protected
  void handleTap() {
...
/// shows a popup menu, consider using [PopupMenuButton].
///
/// A [NFCheckedPopupMenuItem] is 48 pixels high, which matches the default height
/// of a [NFPopupMenuItem]. The horizontal layout uses a [ListTile]; the checkmark
...
/// See also:
///
///  * [NFPopupMenuItem], a popup menu entry for picking a command (as opposed to
///    toggling a value).
///  * [NFPopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [PopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class NFCheckedPopupMenuItem<T> extends NFPopupMenuItem<T> {
...
class _CheckedPopupMenuItemState<T>
    extends NFPopupMenuItemState<T, NFCheckedPopupMenuItem<T>>
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget child) {
        return Transform.translate( // TODO: [NR] add animation variations and rewrite it with [SlideTransition]
...
class _PopupMenuRoute<T> extends PopupRoute<T> {
  ...
  final RelativeRect position;
  final List<NFPopupMenuEntry<T>> items;
  ...
  Widget buildPage(BuildContext context, Animation<double> animation,
    ...
    if (initialValue != null) {
      ...
      for (NFPopupMenuEntry<T> entry in items) {
...
///    semantics.
Future<T> showNFMenu<T>({
  @required BuildContext context,
  @required RelativeRect position,
  @required List<NFPopupMenuEntry<T>> items,
...
/// argument is the value of the [NFPopupMenuItem] that caused its menu to be
/// dismissed.
///
...
typedef PopupMenuItemBuilder<T> = List<NFPopupMenuEntry<T>> Function(
    BuildContext context);
...
///  * [NFCheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
class NFPopupMenuButton<T> extends StatefulWidget {
  /// Creates a button that shows a popup menu.
  ///
  /// The [itemBuilder] argument must not be null.
  const NFPopupMenuButton({
    Key key,
    ...
    this.child,
    this.icon,
    this.buttonSize = Constants.iconButtonSize,
    this.buttonIconSize = Constants.iconSize,
  ...
  /// Size of [NFIconButton]
  final double buttonSize;

  /// Size of icon in [NFIconButton]
...
class _PopupMenuButtonState<T> extends State<NFPopupMenuButton<T>> {
  void showButtonMenu() {
    ...
    final List<NFPopupMenuEntry<T>> items = widget.itemBuilder(context);
